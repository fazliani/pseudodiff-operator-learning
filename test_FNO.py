...
class FNOTester:
    ...
    def generate_data(self):
        """Generate high-frequency data using the paper's method"""
    ...
    def prepare_data(self, x_data, y_data):
        ...
        # Ensure correct dimensions for FNO
        if len(y_data.shape) == 3:  # [batch, height, width]
            y_data = y_data.unsqueeze(1)  # Add channel dimension [batch, channel, height, width]

        if len(x_data.shape) == 3:  # [batch, height, width]
            x_data = x_data.unsqueeze(1)  # Add channel dimension [batch, channel, height, width]
    ...
    def train_fno(self, x_train, y_train, x_test, y_test, n_epochs=50):
        """Train the FNO model"""
        # Initialize FNO with correct number of channels
        model = FNO2d(
            n_modes_height=12,
            n_modes_width=12,
            hidden_channels=32,
            in_channels=1,  # Matches input data channel dimension
            out_channels=1  # Matches output data channel dimension
        ).to(self.device)
    ...
    def plot_comparison(self, model, x_test, y_test, num_samples=5):
        """Plot comparisons between true and predicted solutions"""
        model.eval()
        with torch.no_grad():
        ...
        for i, idx in enumerate(indices):
            ...
            # Compute and display relative L2 error
    ...
    def analyze_frequencies(self, y_true, y_pred):
        """Analyze frequency content of true vs predicted solutions"""
...
def main():
    # Initialize tester
    tester = FNOTester(data_points=150, grid_size=64, truncation_order=10)
    ...
    # Analyze frequencies for one sample
